...
update_current_git_vars() {
    ...
    if [ "$GIT_PROMPT_EXECUTABLE" = "python" ]; then
        ...
        __GIT_STATUS_CMD() {
            git --no-optional-locks status --porcelain --branch 2>/dev/null | ZSH_THEME_GIT_PROMPT_HASH_PREFIX=$ZSH_THEME_GIT_PROMPT_HASH_PREFIX $py_bin "$__GIT_PROMPT_DIR/gitstatus.py"
        }
